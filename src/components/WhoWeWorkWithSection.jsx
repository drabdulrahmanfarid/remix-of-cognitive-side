...
const IndustryCard = ({
  ...
  return <motion.div variants={cardVariants} initial="hidden" animate={isInView ? "visible" : "hidden"} className="w-full">
      <div className="relative rounded-2xl border border-border bg-background overflow-hidden">
        <GlowingEffect spread={80} glow={true} disabled={false} proximity={100} inactiveZone={0.01} borderWidth={1} />
        ...
        <div className="relative z-10 p-6 md:p-8">
          ...
          <div className={`hidden md:grid ${getGridCols(industry.useCases.length)} gap-5`}>
            {industry.useCases.map(useCase => <UseCaseMiniCard key={useCase.title} useCase={useCase} />)}
          </div>
          ...
          <div className="md:hidden">
            ...
            dragFree: true
              ...
              <CarouselContent className="-ml-4">
                {industry.useCases.map(useCase => <CarouselItem key={useCase.title} className="basis-[85%] pl-4">
          ...
          {industry.disclaimer && <p className="text-xs text-muted-foreground/70 mt-6 italic">
              {industry.disclaimer}
            </p>}
        </div>
      </div>
  ...
  return <section ref={ref} className="py-32 md:py-40 px-6 bg-secondary/30 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col gap-8">
          {industriesData.map((industry, index) => <IndustryCard key={industry.title} industry={industry} index={index} isInView={isInView} />)}
        </div>
      </div>
    </section>;
};
