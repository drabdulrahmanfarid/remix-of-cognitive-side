...
const ClosingSection = () => {
  ...
  return <section id="contact" ref={ref} className="min-h-screen py-32 md:py-48 px-6 relative overflow-hidden flex items-center justify-center">
      ...
      <motion.div className="absolute top-8 right-8 z-10 flex items-center gap-2" initial={{
      opacity: 0,
      y: -20
    }} animate={isInView ? {
      opacity: 1,
      ...
      <div className="relative z-10 flex flex-col items-center justify-center text-center">
        ...
        <motion.button onClick={handleBookCall} onMouseEnter={() => setIsButtonHovered(true)} onMouseLeave={() => setIsButtonHovered(false)} className="group relative flex items-center gap-4 mb-12" initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: showSuccess ? 1 : 0,
        ...
        duration: 0.5,
        delay: showSuccess ? 0.15 : 0,
        ease: [0.5, 0, 0, 1]
      }} style={{
        pointerEvents: showSuccess ? 'auto' : 'none'
      }}>
          {/* Left line */}
          <motion.div className="h-px bg-primary/30" animate={{
          width: isButtonHovered ? 48 : 32
        }} transition={{
          duration: 0.3
        }} />
        ...
        <motion.button onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} onClick={handleClick} className="relative cursor-pointer group" initial={{
        opacity: 0,
        y: 40
      }} animate={isInView ? {
        opacity: 1,
        ...
        pointerEvents: isClicked ? 'none' : 'auto'
          ...
          <div className="relative">
            ...
            <motion.div className="absolute -right-8 md:-right-16 top-1/2 -translate-y-1/2" initial={{
            opacity: 0,
            x: -10
          }} animate={{
            ...
            {/* Underline accent */}
            <motion.div className="absolute -bottom-4 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-accent to-transparent" initial={{
            scaleX: 0
          }} animate={{
        ...
        ease: [0.5, 0, 0, 1]
          ...
          {showSuccess ? "15 min intro call" : "Have a project in mind? Let's create something exceptional together."}
        </motion.p>
      </div>

      {/* Email - Bottom */}
      <motion.div className="absolute bottom-8 left-0 right-0 text-center z-10" initial={{
      opacity: 0,
      y: 20
    }} animate={isInView ? {
      opacity: 1,
